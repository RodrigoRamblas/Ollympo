...
class MercadoPagoController extends Controller
{
        ...
        try {
            ...
            if ($httpCode === 201 || $httpCode === 200) {
                ...
                if ($responseData->status === 'authorized') {
                    \Log::info('authorized');
                    $this->createPlanCard(auth()->user()->id, $validated['plan_id']);
                ...
                return response()->json([
                    'status' => 'error',
                    'message' => 'Houve um erro ao processar o pagamento. Contate o administrador caso o erro persistir.',
                    'details' => $errorData,
                ], $httpCode);
            }
        } catch (Exception $e) {
    ...
    public function createPlanCard($userId, $planId)
    {
        try {
            \Log::info('[NOVA ASSINATURA] createPlanCard - userId:' . $userId . ' - planId: ' . $planId);
