...
class SubscriptionsController extends Controller
{
  ...
  public function buy()
  {
    ...
    // Gateway é MercadoPago
    if ($this->request->payment_gateway == 'Mercadopago') {
      return $this->sendMPCheckout();
    }
  ...
  /**
   *  Subscription via MercadoPago
   *
   * @return Response
   */
  protected function sendMPCheckout()
  {
    // Buscar o criador e verificar se o plano está disponível
    $creator = User::whereId($this->request->id)
      ->whereVerifiedId('yes')
      ->firstOrFail();
    ...
    // Gerar o HTML para o checkout
    // TODO - Enviar todos parâmetros:     'creator_id' => $creator->id,      'plan_id' => $plan->id,      'price' => $plan->price,      'interval' => $plan->interval,
    $html = view('checkout.mp_payment', [
      'plan' => $plan,
      'creator' => $creator,
      'creator_id' => $creator->id, // Passando o ID do criador
      'plan_id' => $plan->id, // Passando o ID do plano
      'price' => $plan->price, // Passando o preço do plano
      'interval' => $plan->interval, // Passando o intervalo do plano (ex: mensal, anual)
    ])->render();

    // Retornar o HTML como parte da resposta
    return response()->json([
      'success' => true,
      'html' => $html,
    ]);
