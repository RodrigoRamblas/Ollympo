<div>
    ...
    <ul class="nav nav-pills mb-3 d-flex" id="paymentTab" role="tablist">
        ...
        <li class="nav-item w-50">
            <a class="nav-link rounded-0 text-center font-bold" id="creditCard-tab" data-toggle="pill"
                href="#creditCard" role="tab" aria-controls="creditCard" aria-selected="true">
                <i class="fas fa-credit-card d-inline-block mb-0 mr-2"></i> Cartão
            </a>
        </li>
    </ul>
        ...
        <div class="tab-pane fade" id="creditCard" role="tabpanel" aria-labelledby="creditCard-tab">
            <form id="form-checkout">
                ...
                <div class="form-group mt-3">
                    ...
                    <input type="text" class="form-control mt-2" id="cardholderIdNumber" placeholder="000.000.000-00" value="{{auth()->user()->cpf}}" required>
                    <input type="hidden" class="d-none" id="cardholderIdNumberHidden" />
                </div>
        ...
        <div class="tab-pane fade show active" id="pix" role="tabpanel" aria-labelledby="pix-tab">
            ...
            <div id="pixContainer" class="text-center" style="display: none;">
                <div class="card-body">
                    ...
                    <div class="mb-3">
                        <input type="text" id="pixCode" class="form-control text-center" onclick="copiarTexto()" readonly>
                        <button class="btn btn-outline-primary mt-2" id="copyPixCodeBtn" onclick="copiarTexto()">Copiar PIX</button>
                    </div>
                    <div>
                        <img id="qrcodeImage" style="width: 75%;" alt="QR Code do Pix" onclick="copiarTexto()">
                    </div>
                </div>
            </div>
        </div>
...
<script>
        ...
        const cardForm = mp.cardForm({
            ...
            callbacks: {
                ...
                onSubmit: async (event) => {
                    ...
                    if (!token) {
                        console.error('Token de cartão não gerado');
                        return;
                    }
                    ...
                    const formData = {
                        token: token,
                        creator_id: document.querySelector('input[name="creator_id"]').value,
                        ...
                        email: document.getElementById('form-mp-email').value,
                        identificationType: document.getElementById('cardholderID').value,
                        identificationNumber: document.getElementById('cardholderIdNumberHidden').value
                    };
                    ...
                    try {
                        ...
                        const response = await fetch('/mp/cpaymentplan', {
                            method: 'POST',
                            headers: {
    ...
    $(document).ready(function () {
        ...
        document.getElementById("cardholderIdNumber").addEventListener("input", (event) => {
            const type = document.getElementById("cardholderID").value;
            let value = event.target.value.replace(/\D/g, "");

            document.getElementById("cardholderIdNumberHidden").value = value;
        ...
        // Atualiza placeholder e limpa campos ao mudar tipo
        document.getElementById("cardholderID").addEventListener("change", (event) => {
            const input = document.getElementById("cardholderIdNumber");
            input.value = '';
            input.placeholder = event.target.value === "cpf"
                ? "000.000.000-00"
                : "00.000.000/0000-00";
            document.getElementById("cardholderIdNumberHidden").value = '';
        });

        // Setta valor inicial
        const initialValue = document.getElementById("cardholderIdNumber").value;
        if (initialValue) document.getElementById("cardholderIdNumberHidden").value = initialValue.replace(/\D/g, '');
