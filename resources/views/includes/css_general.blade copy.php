...
@auth
  ...
  <link href="{{ asset('public/js/fileuploader/jquery.fileuploader.min.css')}}" media="all" rel="stylesheet"
    type="text/css" />
  <link href="{{ asset('public/js/fileuploader/jquery.fileuploader-theme-thumbnails.css')}}" media="all" rel="stylesheet"
    type="text/css" />
  <link href="{{ asset('public/js/fileuploader/jquery.fileuploader-theme-dragdrop.css')}}" media="all" rel="stylesheet"
    type="text/css" />
...
<style type="text/css">
  ...
  :root {
    ...
    #f1f1f1 @else #454545 @endif;
    --color-pulse-media-wrapper:
      @if (auth()->check() && auth()->user()->dark_mode == 'off')
    #f8f8f8 @else #373737 @endif;
  }
  ...
  .nav-profile li.active {
    border-bottom: 3px solid
      {{ auth()->check() && auth()->user()->dark_mode == 'on' ? '#fff' : $settings->color_default }}
      !important;
  }
  ...
  .menu-left-home li>a {
    ...
    margin-bottom: 8px;
    border-radius:
      {{ $settings->button_style == 'rounded' ? '20px' : '4px' }}
    ;
    color: #8a96a3;
    -webkit-transition: all 200ms linear;
    -moz-transition: all 200ms linear;
  ...
  .select2-container--default .select2-selection--multiple {
    ...
    background-color:
      @if (auth()->check() && auth()->user()->dark_mode == 'on')
    #303030 !important @else #fff !important @endif;
    border-color:
      @if (auth()->check() && auth()->user()->dark_mode == 'on')
    #222 !important @else #cad1d7 !important @endif;
  }
  ...
  @endif .select2-hidden-accessible {
...
@endif;
  ...
  .icon-dashboard-2 {
    background-color:
      {{ auth()->user()->dark_mode == 'on' ? '#414141' : $settings->color_default . '2e' }}
    ;
    border-radius: 50%;
    color:
      {{ auth()->user()->dark_mode == 'on' ? '#fff' : $settings->color_default }}
      !important;
...
@endif;
  ...
  @endif .file-shop .fileuploader {
    display: block;
    }

  @endauth .count-previews {
    position: absolute;
    right: 10px;
    top: 10px;
    padding: 8px 6px;
  ...
  @endif
    ...
    border: 0 !important;
    background:
      {{ auth()->check() && auth()->user()->dark_mode == 'on' ? '#222' : '#efefef'}}
    ;
    color: #333;
    text-decoration: none;
    margin: 0;
    ...
    .icon--dashboard {
    color: rgb(0 0 0 /
      {{ auth()->check() && auth()->user()->dark_mode == 'on' ? '5%' : '2%' }}
      ) !important;
  ...
  @endif @auth .btn-radio {
    border-radius: 15px !important;
    margin: 0 10px 10px 0 !important;
    border: 1px solid
      {{ auth()->user()->dark_mode == 'on' ? '#5b5a5a' : '#ccc' }}
    ;
    color:
      {{ auth()->user()->dark_mode == 'on' ? '#fff' : '#212529' }}
      !important;
    }
