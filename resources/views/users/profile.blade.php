          ...
          @endif
            ...
            @endif
              ...
              @elseif (auth()->guest() && $totalPosts != 0)
                <a href="{{url('login')}}" data-toggle="modal" data-target="#loginFormModal" class="toggleRegister btn btn-primary btn-profile mr-1">
              ...
              <div class="btn-group">
                @if (auth()->check() && auth()->id() != $user->id && $totalPosts <> 0 && $settings->disable_tips == 'off' && $user->verified_id == 'yes')
                  <a href="javascript:void(0);" data-toggle="modal" title="{{__('general.tip')}}" data-target="#tipForm" class="btn btn-google btn-profile mr-1" data-cover="{{Helper::getFile(config('path.cover').$user->cover)}}" data-avatar="{{Helper::getFile(config('path.avatar').$user->avatar)}}" data-name="{{$user->hide_name == 'yes' ? $user->username : $user->name}}" data-userid="{{$user->id}}">
                    <i class="bi-coin d-block"></i> {{__('general.tip')}}
                  </a>
                @elseif (auth()->guest() && $totalPosts <> 0)
                  <a href="{{url('login')}}" data-toggle="modal" data-target="#loginFormModal" class="toggleRegister btn btn-google btn-profile mr-1" title="{{__('general.tip')}}">
                    <i class="bi-coin d-block"></i> {{__('general.tip')}}
                  </a>
                @endif
                ...
                @if (auth()->check() && auth()->id() != $user->id && $totalPosts <> 0 && $settings->gifts && $user->verified_id == 'yes')
                  <a href="javascript:void(0);" data-toggle="modal" title="{{__('general.gifts')}}" data-target="#giftsForm" class="btn btn-google btn-profile mr-1">
                    <i class="bi-gift d-block"></i> {{ __('general.gifts') }}
                  </a>
                @elseif (auth()->guest() && $totalPosts <> 0)
                  <a href="javascript:void(0);" data-toggle="modal" title="{{__('general.gifts')}}" data-target="#loginFormModal" class="toggleRegister btn btn-google btn-profile mr-1">
                    <i class="bi-gift d-block"></i> {{ __('general.gifts') }}
                  </a>
                @endif
            
                @if (auth()->guest() && $user->verified_id == 'yes' || auth()->check() && auth()->id() != $user->id && $user->verified_id == 'yes')
                  <button @guest data-toggle="modal" data-target="#loginFormModal" @else id="sendMessageUser" @endguest data-url="{{url('messages/'.$user->id, $user->username)}}" title="{{__('general.message')}}" class="toggleRegister btn btn-google btn-profile mr-1">
                    <i class="feather icon-send mr-1 mr-lg-0 d-block"></i> <span class="">{{__('general.message')}}</span>
                  </button>
                @endif
                ...
                @if ($user->verified_id == 'yes')
                  <button class="btn btn-profile btn-google" title="{{__('general.share')}}" id="dropdownUserShare" role="button" data-toggle="modal" data-target=".share-modal">
                    <i class="feather icon-share mr-1 mr-lg-0 d-block"></i> <span class="">{{__('general.share')}}</span>
                  </button>
                @endif
              </div>
              ...
              <div class="card-body px-lg-5 py-lg-5 position-relative">
                ...
                <form method="post" action="{{url('buy/subscription')}}" id="formSubscription">
                  ...
                  @foreach ($allPayment as $payment)
                    ...
                    @if ($payment->name == 'Mercadopago')
                      <div id="mpContainer" class="@if ($allPayment->count() == 1 && $payment->name == 'Mercadopago')d-block @else display-none @endif">
                      </div>
                    @endif
